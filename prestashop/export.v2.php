...
mysqli_query($conn, "SET character_set_results = '$charset', character_set_client = '$charset', character_set_connection = '$charset', character_set_database = '$charset', character_set_server = '$charset'");
...
if (mysqli_errno($conn)) { 
  echo "MySQL error ".mysqli_errno($conn).": ".mysqli_error()."\n<br>When executing:<br>\n$query\n<br>"; 
} 
...
foreach ($arr as $key => $row) {
	...
	//$reduct_result=mysqli_query($conn, $reduct_query); 
	//while ($reduct_row = mysqli_fetch_array($conn, $reduct_result))
	//{
...
//	}
	...
	foreach ($Fields as $key => $value) 
		{
			if ( ($key=="image_url") && ($row['image_url']!="") ){$row[$value]="http://onestep.by/".$row['image_url'].'-large_default/'.$row['plr'].".jpg";}
	
			if ($key=="url"){$row[$value]="http://onestep.by/".$row['clr'].'/'.$row['plr'];}
			
			//.$row['id'].'-'   .($row['ean13'] ? '-'.$row['ean13'] : '').'.html'

			if ($key=='price') {
			  $csv_doc.= ($row[$value]).';'; //intval
			} else {
